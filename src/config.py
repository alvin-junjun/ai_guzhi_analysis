...
class Config:
    ...
    # === 微信支付配置 ===
    wechat_pay_enabled: bool = False          # 是否启用微信支付（商户号 Native 支付）
    wechat_pay_personal_qr_url: Optional[str] = None  # 个人收款码图片 URL，如 /sources/payment/wechat_pay_qrcode.png；与商户支付二选一
    wechat_pay_mchid: Optional[str] = None    # 商户号
    wechat_pay_appid: Optional[str] = None    # 应用ID (小程序/公众号/APP)
    wechat_pay_api_v3_key: Optional[str] = None  # APIv3 密钥
    wechat_pay_cert_serial_no: Optional[str] = None  # 商户证书序列号
    wechat_pay_private_key_path: Optional[str] = None  # 商户私钥路径
    wechat_pay_notify_url: Optional[str] = None  # 支付回调通知地址
    ...
    cookie_domain: Optional[str] = None      # Cookie 域名，如 .guzhiaibot.online，便于 www 与根域共享登录态
    ...
    free_daily_limit: int = 5                # 免费用户每日分析次数
    free_watchlist_limit: int = 10           # 免费用户自选股数量限制
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            wechat_pay_personal_qr_url=os.getenv('WECHAT_PAY_PERSONAL_QR_URL') or None,
    ...
    def get_db_url(self) -> str:
        ...
        支持两种数据库类型:
        - sqlite: 本地 SQLite 数据库（开发环境）
        - mysql: 远程 MySQL 数据库（生产环境）
        ...
        if self.database_type == 'mysql':
            ...
            if not all([self.mysql_host, self.mysql_user, self.mysql_password, self.mysql_database]):
                raise ValueError(
                    "MySQL 配置不完整，请检查 MYSQL_HOST, MYSQL_USER, MYSQL_PASSWORD, MYSQL_DATABASE"
                )
            # 使用 pymysql 驱动
            # 对用户名和密码进行 URL 编码，防止特殊字符（如 #、@、/ 等）导致 URL 解析错误
